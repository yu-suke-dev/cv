...
## 介護経営支援のSaaSにおけるSREとしての業務(2020年)
...
## デザインアセットのバージョン管理アプリのMVP開発のインフラ構築(2020年~)

### 技術要素
GCP, TypeScript, Express, Docker

### 担当業務
- アップロードされたファイル処理をするアプリの設計と実装およびインフラの設計、構築。
...
## スケジュール管理アプリのバックエンドおよび管理画面のバックエンドの実装(2019年)

### 技術要素
- TypeScript, Node.js, Firebase

### 担当業務
- 100種類のクローリングで取得するアプリケーションの設計から実装と定期実行のインフラ設計
- Firebase Projectを複数利用した管理画面のアプリケーションの設計から実装。
- GitHub Actionsを利用したCI/CDの導入。
- Linter(eslint, prettier)の導入。
- TypeScriptとPuppeteerを利用したクローリングのアプリケーションの設計や共通部分の実装を担当。
- GitHub Actionsや、Firestore、全文検索サービスの導入
- アプリの定期実行のインフラ環境の構築。

### 発揮したバリュー
- NoSQL, Firestoreを使用したプロジェクトは初めてだったが、すばやくキャッチアップをし、機能要件や設計の段階からジョインし貢献した。
- Firebaseのプロジェクトを複数使用した安全な管理画面のバックエンドを構築した。
- TypeScriptは初めてだったが、キャッチアップをし、バッチアプリケーションの設計と共通部分の実装および定期実行のためのインフラ設計を行った。

<hr>

## 税理士法人向け顧客管理システムの開発【Java/Spring Framework】(2019年)
